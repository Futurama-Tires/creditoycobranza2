...
class ConciliacionPagosExport implements FromArray, WithColumnFormatting//, WithTitle, ShouldAutoSize, WithStyles, WithColumnWidths
{
    /**
    ...
    public function array(): array
    {
        ...
        $netsuite = Fut_historial_pagos::where('cuenta', 'like', '102.01.01%')
            ->orWhere('cuenta', 'like', '102.01.02%')
            ->orWhere('cuenta', 'like', '102.01.04%')
            ->orWhere('cuenta', 'like', '102.01.05%')
            ->orWhere('cuenta', 'like', '102.01.07%')
            ->orderBy('importe', 'ASC')
            ->orderBy('fecha', 'DESC')
        ...
        foreach ($bancos as $b) {
            ...
            foreach ($netsuite as $key => $n) {
                if ($b->fecha == $n->fecha && abs($b->abonos) == abs($n->importe) && $b->pago == $n->numero_documento) {
                    $conciliados[] = [
                        '' => '',
                        //'Bancos Fecha' => \Carbon\Carbon::parse($b->fecha)->format('d/m/Y'),
                        ...
                        //'Netsuite Fecha' => \Carbon\Carbon::parse($n->fecha)->format('d/m/Y'),
                        ...
                        'Creado Por Netsuite' => $n->creado_por,
        ...
        foreach ($no_conciliados_bancos as $b) {
            ...
            foreach ($no_conciliados_netsuite as $key => $n) {
                ...
                if ($b->fecha == $n->fecha) {
                    ...
                    $condiciones_falladas[] = 'Error Fecha';
                }
        ...
        if (count($conciliados)) {
            ...
            //$header[] = "Error en Monto Fecha o Clave";
            //$header[] = "Error de Cuenta";
            //$header[] = "Error de Metodo";
        ...
        $facturaNombres = collect($data)->skip(2)->pluck(9)->unique()->filter()->values();
        ...
        foreach ($data as $index => $row) {
            ...
            $row[1] = \Carbon\Carbon::createFromFormat('Y-m-d', $row[1])->format('d/m/Y');
            $row[8] = \Carbon\Carbon::createFromFormat('Y-m-d', $row[8])->format('d/m/Y');

            // Rellenar hasta el índice 30
            for ($i = count($row); $i < 33; $i++) {
                $row[$i] = null;
            }
            ...
            if (isset($row[7], $row[12])) {
                if (
                    ($row[7] === 'BBVA' && strpos($row[12], '102.01.01') !== false) ||
                    ($row[7] === 'BBVA USD' && strpos($row[12], '102.01.02') !== false) ||
                    ($row[7] === 'BAJIO CVCA' && strpos($row[12], '102.01.04') !== false) ||
                    ($row[7] === 'BBQ' && strpos($row[12], '102.01.05') !== false) ||
                    ($row[7] === 'INB' && strpos($row[12], '102.01.07') !== false)
                ) {
                    $row[27] = '';
                } else {
                    $row[27] = 'Error Cuenta Bancaria';
                }
            } else {
                $row[27] = 'Error Faltan Valores de Cuenta Bancaria';
            }

            if (isset($row[6], $row[19])) {
                $metodo_banco = trim($row[6]);
                $metodo_sistema = trim($row[19]);
                ...
                if (
                    ...
                    $row[28] = ''; // Método correcto
                } else {
                    $row[28] = 'Error Forma de Pago';
                }
            } else {
                $row[28] = 'Error Faltan valores de Forma de Pago';
            }
            ...
            if (isset($row[20])) {
                ...
                $row[29] = in_array(trim($row[20]), $cfdi_validos) ? '' : 'Error Uso del CFDi';
            } else {
                $row[29] = 'Error sin valor CFDi';
            }

            if (isset($row[25])) {
                $row[30] = (trim($row[25]) === 'CP01-Pago') ? '' : 'Error CFDi para pago';
            } else {
                $row[30] = 'Error sin valor CFDi para pago';
            }
            ...
            if (isset($row[19])) {
                $row[31] = ($row[19] === $forma_pago_primaria) ? '' : 'Error, en info primaria tiene: ' . $forma_pago_primaria;
            }
            // //dd($forma_pago_primaria);
            // if (isset($forma_pago_primaria)) {
            //     $row[31] = $forma_pago_primaria;
            // } else {
            //     $row[31] = "información no encontrada";
            // }

            $nombreFactura = $row[9];
            if (isset($facturasData[$nombreFactura])) {
                $row[23] = $facturasData[$nombreFactura];
            }
        ...
        $data[] = ['', 'Fecha', 'Concepto', 'Pago', 'Abonos', 'Cliente']; // encabezados
        foreach ($final_no_conciliados_bancos as $b) {
            $fecha = \Carbon\Carbon::createFromFormat('Y-m-d', $b->fecha)->format('d/m/Y');

            $data[] = ['', $fecha, $b->pago, $b->abonos, $b->concepto_referencia, $b->cliente,];
        }
        ...
        foreach ($errores as $err) {
            ...
            foreach ($final_no_conciliados_netsuite as $no_con) {
                if (($importeError == $no_con->importe) && ($err[11] == $no_con->nombre)) {
                    // Log::info('Duplicado encontrado', [
                    //     'Conciliado Num Doc' => $con['Numero Documento Netsuite'] ?? 'N/A',
                    //     'No Conciliado Num Doc' => $no_con->numero_documento
                    // ]);
                    $no_con->posible_duplicado = "Posible Duplicado de " . $err[10];
